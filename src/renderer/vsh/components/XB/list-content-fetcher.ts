...
export async function getSettingsContent(key: string, settings: UserSettings): Promise<CategoryContent> {
	switch (key) {
		...
		case "music":
			return {
				content: [
					{ id: "system.settings:music.preferred_library", name: "Preferred Library", desc: "Sets the preferred music library content listed under the [Music] category.", Icon: "icon:settings.item", /* value: await manager.get("music.preferred_library") */ value: settings?.music?.preferred_library },
					{
					...
					{ id: "system.settings:music.crossfade", name: "Crossfade Playback", desc: "Sets the amount of time that is crossfaded (overlapped) between music tracks. This setting is used when playing content other than CD Audio.", Icon: "icon:settings.item", /* value: "Off" */ /* value: await manager.get("music.crossfade") || "Off" */ value: settings?.music?.crossfade == 0 ? "Off" : `${settings?.music?.crossfade} Seconds` },
					{ id: "system.settings:music.output_freq", name: "Output Frequency", desc: "Sets the audio output frequency when playing music content.\nRecommended: Auto or Upsample", Icon: "icon:settings.item", /* value: "Auto" */ value: settings?.music?.output_freq },
					// { id: "system.settings:music.bitmapping", name: "Bitmapping", desc: "Honestly I care about audio but even I don't know what this does. “Set this according to your preference”??? HOW? IT WAS SUPER PAINFUL TO CHANGE MUSIC SETTINGS BETWEEN PLAYING TRACKS ON THE PS3!", Icon: "icon:settings.item", value: "Type 1" },
				]
			};
	}
	return { content: [] };
...
export async function getXMLListContent(key: string) {
	...
	const data = /* await */ fetch(`xb://localhost/data/${key.replace(/\./g, "/")}.xml?t=${Date.now()}`)
		.then(res => res.text())
