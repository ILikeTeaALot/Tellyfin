...
import { baseWindow, mainWindow, themeBackgroundWindow, uiWindow } from "./important-values";
...
const createWindow = (): void => {
	...
	baseWindow.value = new BaseWindow({
		width: 1920,
		height: 1080,
		fullscreen: true,
		resizable: false,
	...
	baseWindow.value.on("close", () => {
		app.quit();
	});

	themeBackgroundWindow.value = new BrowserWindow({
		parent: baseWindow.value,
		width: 1920,
		height: 1080,
		fullscreen: true,
		resizable: false,
	...
	themeBackgroundWindow.value.on("close", () => {
		app.quit();
	});

	uiWindow.value = new BrowserWindow({
		parent: themeBackgroundWindow.value,
		width: 1920,
		height: 1080,
		fullscreen: true,
		resizable: false,
	...
	mainWindow.value = uiWindow.value.webContents;

	uiWindow.value.on("close", () => {
		app.quit();
	});

	// and load the index.html of the app.
	uiWindow.value.webContents.loadURL(MAIN_WINDOW_WEBPACK_ENTRY);

	// Open the DevTools.
	uiWindow.value.webContents.openDevTools();

	setupMPV(baseWindow.value);
